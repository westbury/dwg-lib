...
//                long y = (sizeOfGraphicImageInBytes + 1) * 4;
                
                long type = dataStream.getRL();
//                long type2 = dataStream.getRL();
//                int p = dataStream.position();
//                for (int o = 0; o < 32; o++) {
//                    dataStream.position(p+o);
                for (int i = 0; i < sizeOfGraphicImageInBytes; i++) {
                    int c = dataStream.getRC();
                    System.out.print(Integer.toHexString(c<0 ? c+256 : c) + " ");
                }
//                System.out.println("");
//                System.out.println("");
//                }
...
//            if (!fileVersion.is2013OrLater() && EntityObject.this instanceof PlaneSurface) {
//                boolean x = dataStream.getB();
////                boolean y = dataStream.getB();
...
//            }
            ...
            if (myLayer instanceof Dictionary || (myLayer instanceof GenericObject && ((GenericObject)myLayer).objectType.equals("MATERIAL"))) {
                // try next handle
                layerHandle = handleStream.getHandle();
